...
class Beneficiario {
    ...
    static add(data) {
        ...
        const sheet = ss.getSheetByName(WA_SHEET_BENEFICIARI);
        if (!sheet)
            throw new Error("Foglio '" + WA_SHEET_BENEFICIARI + "' non trovato.");
        ...
        const idx = Beneficiario._getColIndexes(headers);
        if (idx.nome >= 0)
            sheet.getRange(newRowIdx, idx.nome + 1).setValue(data.nome.trim());
        if (idx.cognome >= 0)
            sheet.getRange(newRowIdx, idx.cognome + 1).setValue(data.cognome.trim());
        if (idx.cf >= 0)
            sheet.getRange(newRowIdx, idx.cf + 1).setValue(targetCF);
        if (idx.dataNascita >= 0)
            sheet.getRange(newRowIdx, idx.dataNascita + 1).setValue(data.dataNascita || "");
        if (idx.luogoNascita >= 0)
            sheet.getRange(newRowIdx, idx.luogoNascita + 1).setValue(data.luogoNascita.trim());
        if (idx.email >= 0)
            sheet.getRange(newRowIdx, idx.email + 1).setValue(data.email.trim());
        if (idx.telefono >= 0)
            sheet.getRange(newRowIdx, idx.telefono + 1).setValue(data.telefono.trim());
        if (idx.statoLavorativo >= 0)
            sheet.getRange(newRowIdx, idx.statoLavorativo + 1).setValue(data.statoLavorativo);
        if (idx.titoloStudio >= 0)
            sheet.getRange(newRowIdx, idx.titoloStudio + 1).setValue(data.titoloStudio);
        if (idx.cittadinanza >= 0)
            sheet.getRange(newRowIdx, idx.cittadinanza + 1).setValue(data.cittadinanza.trim());
        if (idx.residenza >= 0)
            sheet.getRange(newRowIdx, idx.residenza + 1).setValue(data.residenza.trim());
        if (idx.domicilio >= 0)
            sheet.getRange(newRowIdx, idx.domicilio + 1).setValue(data.domicilio.trim());
        if (idx.inCaricoServizi >= 0)
            sheet.getRange(newRowIdx, idx.inCaricoServizi + 1).setValue(data.inCaricoServizi);
